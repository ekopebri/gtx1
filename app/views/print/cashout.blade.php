...
<div class="row">
	<div class="col-lg-12">
          <h3 class="sub-header">Cash Out Bulan <?php $monthNum = $month; $monthName = date("F", mktime(0, 0, 0, $monthNum, 10)); echo $monthName; //output: May?> {{$year}} (Dalam jutaan)</h3>
            ...
            {{ Form::open(array('action' => 'KonsolidasiController@omzet')) }}
				<table class="table table-bordered">
					...
					<tr>
						...
						<td>{{ number_format($ok_netto_divisi) }}</td>
						<td>{{ number_format($proyeksi_proyek + $proyeksi_divisi) }}</td>
						<td>{{ number_format($jan_proyek + $jan_divisi) }}</td>
						<td>{{ number_format($feb_proyek + $feb_divisi) }}</td>
						<td>{{ number_format($mar_proyek + $mar_divisi) }}</td>
						<td>{{ number_format($apr_proyek + $apr_divisi) }}</td>
						<td>{{ number_format($mei_proyek + $mei_divisi) }}</td>
						<td>{{ number_format($jun_proyek + $jun_divisi) }}</td>
						<td>{{ number_format($jul_proyek + $jul_divisi) }}</td>
						<td>{{ number_format($agt_proyek + $agt_divisi) }}</td>
						<td>{{ number_format($sep_proyek + $sep_divisi) }}</td>
						<td>{{ number_format($okt_proyek + $okt_divisi) }}</td>
						<td>{{ number_format($nov_proyek + $nov_divisi) }}</td>
						<td>{{ number_format($des_proyek + $des_divisi) }}</td>
					</tr>
				</table>
			{{Form::close() }}	</div>
</div>
